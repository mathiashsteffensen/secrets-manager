...
It provides a CLI tool for editing and encrypting a secrets.yml.enc file (or whatever file name you want to configure,
more on that later). As well as a Go API for accessing this file in your application. The Go API also provides options
to load other unencrypted yaml files to make all configuration accessible from one place.
...
This command opens your decrypted secrets file for editing in the editor saved in the EDITOR env variable, by default it
will attempt to open this file in the sublime text editor,
...
```bash
go get github.com/mathiashsteffensen/secrets-manager
```

Documentation: https://pkg.go.dev/github.com/mathiashsteffensen/secrets-manager/app_config

This project follows semantic versioning and you can expect the public API to be frozen as long as you lock the major
version. Read more about semantic versioning [here](https://semver.org/).
